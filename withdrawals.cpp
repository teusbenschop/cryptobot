  ...
  if (arguments.size() == 2) {
    ...
    if (in_array (exchange, exchanges)) {
      ...
      if (in_array (coin, coins)) {
        ...
        exchange_withdrawalhistory (exchange, coin, order_ids, coin_abbreviations, coin_ids, quantities, addresses, transaction_ids);
        for (unsigned int i = 0; i < order_ids.size(); i++) {
          if (coin_abbreviations[i] != coin_abbreviation) continue;
          cout << "order " << order_ids[i] << " withdraw " << quantities[i] << " " << coin_ids[i] << " to " << addresses[i] << " txid " << transaction_ids[i] << endl;
        }
    ...
    cout << "Usage: ./withdrawals exchange coin" << endl;
  }
