...
int main (int argc, char *argv[])
{
  ...
  if (arguments.size() == 3) {
    ...
    if (in_array (exchange, exchanges)) {
      ...
      if (in_array (market, markets)) {
        ...
        if (in_array (coin, coins)) {
          ...
          // Read and display the buy orders for this coin.
          vector <float> quantity;
          vector <float> rate;
          exchange_get_buyers_via_satellite (exchange, market, coin, quantity, rate, false, nullptr);
          for (unsigned int i = 0; i < quantity.size(); i++) {
            to_tty ({"Buy", float2string (quantity[i]), coin, "at rate", float2string (rate[i]), "total", to_string (quantity[i] * rate[i]), market});
          }
    ...
    to_tty ({"Usage: ./buyers exchange market coin"});
  }
